...
PixelShader =
{
	...
	[[
		...
		float3 ApplyCeruleanGlow(float3 Color, float3 WorldSpacePos, float FogOfWarAlphaValue)
		{
			float2 Warp;
			...
			float3 GlowColor = lerp(DeepBlue, Cerulean, CombinedPulse);

			float GlowStrength = (0.03f + CombinedPulse * 0.08f + Spark * 0.12f) * HeightMask * (0.4f + TerrainNoise);
