...
PixelShader =
{
	...
	[[
		...
		float3 ApplyCharoGlow(float3 Color, float3 WorldSpacePos, float FogOfWarAlphaValue)
		{
			float2 Warp;
			Warp.x = FBM(WorldSpacePos.xz * 0.02f);
			Warp.y = FBM((WorldSpacePos.xz + 1000.0f) * 0.02f);
			...
			float RegionalVariation = FBM(WorldSpacePos.xz * 0.006f);
			RegionalVariation = smoothstep(0.30f, 0.70f, RegionalVariation);

			float PulseA = 0.5f + 0.5f * sin(GlobalTime * 0.60f + TerrainNoise * 12.0f);
			float PulseB = 0.5f + 0.5f * sin(GlobalTime * 1.32f + TerrainNoise * 31.0f);
			float Drift = 0.5f + 0.5f * sin(GlobalTime * 0.04f + TerrainNoise * 120.0f);
			float CombinedPulse = PulseA * PulseB * Drift;

			float Spark = pow(saturate(sin(GlobalTime * 0.12f + TerrainNoise * 500.0f)), 24.0f);
			float HeightMask = saturate(1.0f - (WorldSpacePos.y / 300.0f));

			float3 DeepRed = float3(0.08f, 0.005f, 0.01f);
			float3 Red = float3(0.55f, 0.04f, 0.08f);
			float3 Pink = float3(1.00f, 0.65f, 0.72f);

			float3 RegionalColor = lerp(Red, Pink, RegionalVariation);
			float3 GlowColor = lerp(RegionalColor, Pink, CombinedPulse * 0.25f);
			float GlowStrength = (0.02f + CombinedPulse * 0.04f + Spark * 0.08f) * HeightMask * (0.4f + TerrainNoise);

			Color += GlowColor * GlowStrength * 5.0f;
	...
	MainCode PS_leaf
	{
		...
		[[
			PDX_MAIN
			{
				...
				Color = ApplyCharoGlow(Color, Input.WorldSpacePos, 1.0f);
